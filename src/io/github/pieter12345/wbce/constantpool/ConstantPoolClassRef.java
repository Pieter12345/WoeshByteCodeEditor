...
public class ConstantPoolClassRef implements ConstantPoolObject {
	
	// Variables & Constants.
	private int index;
	
	// Constructor.
	public ConstantPoolClassRef(int index) {
		this.index = index;
	}
	
	@Override
	public ConstantPoolType getType() {
		return ConstantPoolType.CLASS_REF;
	}
	
	public static ConstantPoolObject fromInputStream(FancyInputStream inStream) throws IOException {
		return new ConstantPoolClassRef(inStream.readTwoByteInt());
	}
	...
	public String val(ClassConstantPool constPool) {
		return "Class reference index: " + this.index + "->"
				+ (constPool != null && constPool.size() >= this.index ? constPool.get(this.index).val(constPool) : "~UNKNOWN VALUE");
	}
	
	@Override
	public String simpleVal(ClassConstantPool constPool) {
		return (constPool != null && constPool.size() >= this.index ? constPool.get(this.index).simpleVal(constPool) : "~UNKNOWN_VALUE");
	}
}
